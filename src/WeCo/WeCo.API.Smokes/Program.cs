...
Action<ResourceBuilder> configureResource = r => r.AddService(
    "WeCo.API.Smokes", serviceVersion: assemblyVersion, serviceInstanceId: Environment.MachineName);
...
builder.Services.AddOpenTelemetryTracing((options) => {
    options
        ...
        .AddSource("WeCo.API.Smokes.ActivitySource")
...
var activitySource = new ActivitySource("WeCo.API.Smokes.ActivitySource");
var meter = new Meter("SmokesMetrics");
...
app.MapGet("/", async (ILogger<Program> logger, IHttpClientFactory httpClientFactory) => {
    ...
    using (var activity = activitySource.StartActivity("Get Smoke Sensor")) {
